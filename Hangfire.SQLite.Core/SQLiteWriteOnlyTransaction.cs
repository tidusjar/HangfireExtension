...
namespace Hangfire.SQLite.Core
{
    internal class SQLiteWriteOnlyTransaction : JobStorageTransaction
    {
        ...
        {
            var addAndSetStateSql = string.Format(@"
...
values (@jobId, @name, @reason, @createdAt, @data)", _storage.GetSchemaName());
        ...
        {
            QueueCommand(x => x.Execute(
                string.Format(@"insert into [{0}.Counter] ([Key], [Value]) values (@key, @value)",
        ...
        public override void IncrementCounter(string key, TimeSpan expireIn)
        {
            QueueCommand(x => x.Execute(
                string.Format(
                    @"insert into [{0}.Counter] ([Key], [Value], [ExpireAt]) values (@key, @value, @expireAt)",
                    _storage.GetSchemaName()),
...
//when not matched then insert ([Key], Value, Score) values (Source.[Key], Source.Value, Source.Score);", _storage.GetSchemaName());
            ...
            {
                var tableName = string.Format("[{0}.Set]", _storage.GetSchemaName());
                var selectSqlStr = string.Format("select * from {0} where [Key] = @key and Value = @value", tableName);
                var insertSqlStr = string.Format("insert into {0} ([Key], Value, Score) values (@key, @value, @score)",
                    tableName);
                var updateSqlStr = string.Format("update {0} set Score = @score where [Key] = @key and Value = @value ",
                    tableName);
        ...
        {
            var query = string.Format(@"delete from [{0}.Set] where [Key] = @key and Value = @value",
                _storage.GetSchemaName());
        ...
        public override void InsertToList(string key, string value)
        {
            AcquireListLock();
            QueueCommand(x => x.Execute(
                string.Format(@"insert into [{0}.List] ([Key], Value) values (@key, @value);",
                    _storage.GetSchemaName()),
                new {key, value}));
        }
        ...
        public override void RemoveFromList(string key, string value)
        {
            AcquireListLock();
            QueueCommand(x => x.Execute(
                string.Format(@"delete from [{0}.List] where [Key] = @key and Value = @value",
                    _storage.GetSchemaName()),
                new {key, value}));
        }
...
//delete from cte where row_num not between @start and @end", _storage.GetSchemaName());
            ...
            var trimSql = string.Format(@"
delete from [{0}.List] where [Key] = @key and Id not in (
  select Id from [{0}.List] where [Key] = @key order by Id desc limit @limit offset @offset)",
                _storage.GetSchemaName());

            AcquireListLock();
            QueueCommand(x => x.Execute(
                trimSql,
                new {key, limit = keepEndingAt - keepStartingFrom + 1, offset = keepStartingFrom}));
        }
...
//when not matched then insert ([Key], Field, Value) values (Source.[Key], Source.Field, Source.Value);", _storage.GetSchemaName());
        ...
        public override void RemoveHash(string key)
        {
            ...
            var query = string.Format(@"delete from [{0}.Hash] where [Key] = @key", _storage.GetSchemaName());
...
values (@key, @value, 0.0)", _storage.GetSchemaName());
            ...
            AcquireSetLock();
            QueueCommand(x => x.Execute(query, items.Select(value => new {key, value}).ToList()));
        }
        ...
        public override void RemoveSet(string key)
        {
            ...
            var query = string.Format(@"delete from [{0}.Set] where [Key] = @key", _storage.GetSchemaName());
...
update [{0}.Hash] set ExpireAt = @expireAt where [Key] = @key", _storage.GetSchemaName());
            ...
            AcquireHashLock();
            QueueCommand(x => x.Execute(query, new {key, expireAt = DateTime.UtcNow.Add(expireIn)}));
        }
